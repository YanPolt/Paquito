    ...
    axios
      ...
      .then((res) => {
        ...
        if (fecha.desde !== -1 && fecha.hasta !==-1 && fecha.desde !== '' && fecha.hasta !=='') {
          ...
          if(desde <= hasta){
            datos = datos.filter((item)=> (Date.parse(item["Fecha.de.Firma.del.Contrato"])>=desde && Date.parse(item["Fecha.de.Firma.del.Contrato"])<=hasta));
          }  
        }
        console.log(datos);
        setData(datos);
        ...
        <InputGroup className="d-flex align-items-center justify-content-center">
          <Form onSubmit={enviarDatos} className="drop">
            ...
            <Form onSubmit={enviarDatos}>
              <Dropdown>
                <Dropdown.Toggle className="customButton" variant="info">Añadir Filtros <FontAwesomeIcon icon={faFilter}></FontAwesomeIcon></Dropdown.Toggle>
                <Dropdown.Menu className="col-md-6">
                  ...
                  <Button className="col-4 mr-4" variant="danger" onClick={reiniciar} type="reset">Cancelar</Button>
                  <Button className="col-4 customButton" variant="light" type="submit">Filtrar</Button>
                  </div>
                </Dropdown.Menu>
              </Dropdown>
              </Form>
